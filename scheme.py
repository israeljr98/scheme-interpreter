...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) < len(vals):
            raise SchemeError('number of values exceeds number of parameters')
        elif len(formals)  > len(vals):
            raise SchemeError('number of parameters exceeds number of values')
        else:
            while formals != nil:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Incorrect number of arguments were passed in.')        
        #
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # 10
        name_of_func = target.first
        parameter = target.second
        body_of_func = expressions.second
        env.define(name_of_func, LambdaProcedure(parameter, body_of_func, env))
        return name_of_func
        # 
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        let_env.define(bindings.first.first, scheme_eval(bindings.first.second.first, env))
